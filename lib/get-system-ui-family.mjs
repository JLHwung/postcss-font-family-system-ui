import compatData from "./stats.json";
...
export default function getSystemUiFamily(supportedBrowsers, preserve) {
	const polyfillFlags = generatePolyfillFlags(compatData, supportedBrowsers);
	...
	return result.concat.apply(
		result,
		polyfill.filter(([flag]) => polyfillFlags[flag]).map((v) => v[1])
	);
}
